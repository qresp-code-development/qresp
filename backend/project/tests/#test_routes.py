...
class RouteTests(unittest.TestCase):
    ...
    def setUp(self):
        ...
        serverform = ServerForm(
            **{'hostUrl': 'https://notebook.rcc.uchicago.edu/files'})
        chartform = {'id': 'c0', 'caption': 'chart 1', 'number': '1', 'files': 'charts/figure1/figure1.csv, charts/figure1/figure1.ipynb, charts/figure1/figure1.jpg',
                     'imageFile': 'charts/figure1/figure1.jpg', 'notebookFile': 'charts/figure1/figure1.ipynb', 'properties': 'potential energy, band gap'}
        toolform = {'id': 't0', 'kind': 'software', 'packageName': 'West', 'URLs': '', 'version': '3.0.0', 'programName': 'wstat.x',
                    'patches': 'tools/modded_qbox.diff', 'description': 'Modified west code', 'facilityName': '', 'measurement': ''}
        datasetform = {'id': 'd0', 'files': 'datasets/datasetA.dat, datasets/datasetB.dat',
                       'readme': 'DAT files', 'URLs': ''}
        scriptform = {'id': 'd0', 'files': 'datasets/datasetA.dat, datasets/datasetB.dat',
                      'readme': 'DAT files', 'URLs': ''}
    ...
    def test_postMethods(self):
        for pagename in self.postPageForms.keys():
            ...
            if 'charts' in pagename or 'tools' in pagename or 'scripts' in pagename or 'datasets' in pagename \
                ...
                response = self.client.post(pagename, data=json.dumps(
                ...
                response = self.client.post(pagename, data=json.dumps(
                    form), content_type='application/json')
            else:

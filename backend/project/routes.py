...
def uploadFile():
    ...
    try:
        ...
        session[CURATOR_FIELD.CHARTS] = [
            form.data for form in paperform.charts.entries]
        session[CURATOR_FIELD.TOOLS] = [
            form.data for form in paperform.tools.entries]
        session[CURATOR_FIELD.DATASETS] = [
            form.data for form in paperform.datasets.entries]
        session[CURATOR_FIELD.SCRIPTS] = [
            form.data for form in paperform.scripts.entries]
        session[CURATOR_FIELD.HEADS] = [
            form.data for form in paperform.heads.entries]
        session[CURATOR_FIELD.EDGES] = [
            form.data for form in paperform.workflow.edges.entries]
...
def qrespcurator():
    ...
    if request.method == 'GET':
        ...
        detailsform = DetailsForm(
            **session.get(CURATOR_FIELD.DETAILS, request.form))
        # Renders the servers section
        serverform = ServerForm(
            **session.get(CURATOR_FIELD.SERVERS, request.form))
        serverslist = Servers()
        ...
        projectform = ProjectForm(
            **session.get(CURATOR_FIELD.PROJECT, request.form))
        ...
        referenceform = ReferenceForm(
            **session.get(CURATOR_FIELD.REFERENCE, request.form))
        ...
        documentationform = DocumentationForm(
            **session.get(CURATOR_FIELD.DOCUMENTATION, request.form))
...
def getTreeInfo():
    ...
    services = []
    try:
        content_path = pathDetails.get("treeUrl", session.get(
            "project", {}).get("fileServerPath", ""))
...
def charts():
    ...
    if request.method == 'POST' and chartform.validate():
        ...
        chartList = deepcopy(session.get(CURATOR_FIELD.CHARTS, []))
        chartList = [genid.addId(item) for item in chartList if item.get(
            'id') not in chartform.id.data]
...
def tools():
    ...
    if request.method == 'POST' and toolform.validate():
        ...
        toolList = deepcopy(session.get(CURATOR_FIELD.TOOLS, []))
        toolList = [genid.addId(item) for item in toolList if item.get(
            'id') not in toolform.id.data]
...
def datasets():
    ...
    if request.method == 'POST' and datasetform.validate():
        ...
        datasetList = deepcopy(session.get(CURATOR_FIELD.DATASETS, []))
        datasetList = [genid.addId(item) for item in datasetList if item.get(
            'id') not in datasetform.id.data]
...
def scripts():
    ...
    if request.method == 'POST' and scriptform.validate():
        ...
        scriptList = deepcopy(session.get(CURATOR_FIELD.SCRIPTS, []))
        scriptList = [genid.addId(item) for item in scriptList if item.get(
            'id') not in scriptform.id.data]
...
def addToWorkflow():
    ...
    nodesList = []
    edgeList = []
    workflow = WorkflowCreator(session.get(
        CURATOR_FIELD.PROJECT, {}).get("fileServerPath", ""))
...
def download():
    ...
    fileServerPath = session.get(
        CURATOR_FIELD.PROJECT, {}).get("fileServerPath", "")
    notebookPath = session.get(
        CURATOR_FIELD.PROJECT, {}).get("notebookPath", "")
    downloadPath = session.get(
        CURATOR_FIELD.PROJECT, {}).get("downloadPath", "")
    ...
    try:
        ...
        if not projectName:
            ...
            previewFolder = serverpathList[len(
    ...
    with open("papers/"+previewFolder+"/"+"data.json", "w") as outfile:
        json.dump(paperdata, outfile, default=lambda o: o.__dict__,
                  separators=(',', ':'), sort_keys=True, indent=3)
...
def publish():
    ...
    if request.method == 'POST' and form.validate():
        ...
        if len(error) > 0:
            ...
            googleauth = GoogleAuth(Config.get_setting(app.config['env'], 'GOOGLE_CLIENT_ID'), Config.get_setting(
                app.config['env'], 'REDIRECT_URI'), Config.get_setting('GOOGLE_API', 'SCOPE'))
            google = googleauth.getGoogleAuth()
            auth_url, state = google.authorization_url(
                Config.get_setting('GOOGLE_API', 'AUTH_URI'), access_type='offline')
...
def authorized():
    ...
    if 'code' not in request.args and 'state' not in request.args:
        print('denied access.')
        return redirect(url_for('qrespcurator'))
    googleauth = GoogleAuth(Config.get_setting(
        app.config['env'], 'GOOGLE_CLIENT_ID'), Config.get_setting(app.config['env'], 'REDIRECT_URI'))
                        ...
                        if response.status_code == 400:
                            ...
                            try:
                                ...
                                mailserver = smtplib.SMTP(
                                    'smtp.gmail.com', 587)
                                mailserver.starttls()
                                mailserver.login(Config.get_setting(
                                    'GLOBAL', 'MAIL_ADDR'), Config.get_setting('SECRETS', 'MAIL_PWD'))
